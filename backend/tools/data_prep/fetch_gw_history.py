...
Run with: python -m tools.data_prep.fetch_gw_history  (from the backend/ folder)
"""
...
# ("2025_26", e.g. fpl.config's archive_season) - this is vaastav's repo's
# own directory naming, not something we control, so it can't be unified
# away. The .replace() below is the one conversion point back to our own
# underscore convention for the output filename.
SEASON = "2025-26"
URL = f"https://raw.githubusercontent.com/vaastav/Fantasy-Premier-League/master/data/{SEASON}/gws/merged_gw.csv"
OUT_PATH = DATA_DIR / f"gw_history_{SEASON.replace('-', '_')}.csv"
